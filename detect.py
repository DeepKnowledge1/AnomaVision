...
def parse_args():
    ...
    parser.add_argument('--show_first_batch_only', action='store_true', default=True,
                        help='Show visualization only for the first batch.')
    parser.add_argument('--viz_alpha', type=float, default=0.5,
                        help='Alpha value for heatmap overlay.')
    parser.add_argument('--viz_padding', type=int, default=40,
                        help='Padding for boundary visualization.')
    parser.add_argument('--viz_color', type=str, default='128,0,128',
...
def save_visualization(images, filename, output_dir):
    ...
    if len(images.shape) == 4:  # Batch of images
        for i, img in enumerate(images):
            individual_filepath = os.path.join(output_dir, f"{filename.split('.')[0]}_batch_{i}.png")
