...
AnomaVision can compare the feature representation seen in production with a
trusted reference population. The monitor is an **additive observer**: anomaly
scores, localization, PatchCore/PaDiM behavior, and hardware inference paths are
not changed by enabling monitoring.

### 1. Generate a reference embedding set

Use healthy/reference images from the same operating population used to train
or validate the deployed model:
...
The command writes the embedding matrix and a JSON metadata sidecar. Reference
embeddings should come from trusted, representative normal data; do not build
the baseline from already-drifted production data.

### 2. Enable monitoring during inference
...
The monitor keeps only a bounded rolling production window. Before enough
samples arrive it reports `warming_up`; once ready it reports `stable` or
`drift` and writes the latest machine-readable status to the configured JSON
file.

For supported PyTorch representations such as PatchCore, the production monitor
reuses the representation captured during normal inference rather than running
the backbone a second time. If a backend does not expose a drift representation,
monitoring is skipped safely and anomaly inference continues.

A drift warning is a signal to investigate changes in the production input
population. It does not by itself indicate that an image is anomalous.
