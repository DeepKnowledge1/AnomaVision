...
import anomavision
from anomavision.actions.integration import (
...
def create_parser(add_help: bool = True) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model", type=str, default=None, help="Model file (.pt for PyTorch, .onnx for ONNX, .engine for TensorRT)")
    parser.add_argument("--device", type=str, default=None, choices=["auto", "cpu", "cuda"], help="Device to run inference on (auto will choose cuda if available)")
...
def run_inference(args):
    ...
    # Industrial actions are optional. Existing inference/classification remains the source of truth.
    action_dispatcher = None
    actions_config = config.get("actions")
    if actions_config:
        logger.info("Industrial actions enabled: %d action(s)", len(actions_config))
        action_dispatcher = create_action_dispatcher(actions_config, logger=logger)
        ...
        for batch_idx, (batch, images, _, _) in enumerate(test_dataloader):
            ...
            with profilers["postprocessing"]:
                try:
                    ...
                    # Dispatch the already-classified result. No second decision engine is used.
                ...
                with profilers["visualization"]:
                    try:
                        boundary_images = anomavision.visualization.framed_boundary_images(
                            images, localization_masks, is_anomaly, padding=config.get("viz_padding", 40)
                        )
                        heatmap_images = anomavision.visualization.heatmap_images(
                            images, score_maps, masks=localization_masks, alpha=config.get("viz_alpha", 0.5)
                        )
                        highlighted_images = anomavision.visualization.highlighted_images(
                            [images[i] for i in range(len(images))], localization_masks, color=viz_color
                        )
