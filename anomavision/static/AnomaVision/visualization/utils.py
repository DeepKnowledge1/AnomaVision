...
def merge_images(images: np.ndarray, margin: int = 0) -> np.ndarray:
    """
...
def frame_image(
    image: np.ndarray, padding: int = 30, color: Tuple[int, int, int] = (0, 0, 0)
) -> np.ndarray:
    """
...
def composite_image(
    image_one: np.ndarray, image_two: np.ndarray, mask: np.ndarray
) -> np.ndarray:
    """
