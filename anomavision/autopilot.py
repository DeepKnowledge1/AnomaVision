...
def _format_metric(value: Any, digits: int = 4) -> str:
    """Backward-compatible public formatter used by the autopilot tests/API."""
...
def _profile_model(
    ...
    maps = (
        np.asarray(maps_all, dtype=np.float32)
        if maps_all
    ...
    if len(np.unique(labels)) > 1:
        ...
        threshold, threshold_f1 = (
            float(np.median(scores)) if len(scores) else 0.0
        ), 0.0
    metrics = compute_metrics(labels, scores, thresh=threshold) if len(labels) else {}
    pixel_auroc = None
    localization = {
        "available": bool(len(maps) == len(labels) and maps.ndim == 3),
    ...
    if (
        ...
        and masks.shape == maps.shape
        and np.unique(masks).size > 1
    ):
        try:
            pixel_auroc = float(
                roc_auc_score(masks.reshape(-1) > 0.5, maps.reshape(-1))
            )
        except ValueError:
            pass
        loc_masks = make_localization_mask(
            maps, (scores >= threshold).astype(np.uint8)
        ).astype(bool)
        ...
        if anomaly.any():
            localization["anomaly_non_empty_fraction"] = float(
                non_empty[anomaly].mean()
            )
            localization["anomaly_mean_mask_area_fraction"] = float(
                area[anomaly].mean()
            )
        if normal.any():
            localization["normal_false_positive_fraction"] = float(
                non_empty[normal].mean()
            )
    ...
    metrics["image_auroc"] = (
        float(metrics["auc_score"]) if metrics.get("auc_score") is not None else None
    )
...
def _select(
    ...
    if target_latency_ms is not None:
        eligible = {
            ...
            if r["latency_ms"]["p95"] <= target_latency_ms
        }
    if not eligible:
        eligible = results
    return max(
        eligible,
        key=lambda n: (
            eligible[n]["metrics"].get("image_auroc") or 0.0,
            -eligible[n]["latency_ms"]["p95"],
        ),
    )
...
def _write_report(manifest: Dict[str, Any], output_dir: Path) -> None:
    ...
    for name, result in manifest["candidates"].items():
        ...
        cards.append(
            f"""<article class="card {"selected" if active else ""}"><div class="top"><b>{html.escape(name.upper())}</b><span class="badge">{"SELECTED" if active else "CANDIDATE"}</span></div><div class="big">{_fmt(metrics.get("image_auroc"))}<small>Image AUROC</small></div><div class="stats"><div><b>{_fmt(metrics.get("pixel_auroc"))}</b><small>Pixel AUROC</small></div><div><b>{result["latency_ms"]["p95"]:.1f} ms</b><small>P95 latency</small></div><div><b>{_pct(loc.get("anomaly_non_empty_fraction"))}</b><small>Anomaly coverage</small></div></div></article>"""
        )
        rows.append(
            f"""<tr class="{"active" if active else ""}"><td><strong>{html.escape(name)}</strong></td><td>{_fmt(metrics.get("image_auroc"))}</td><td>{_fmt(metrics.get("pixel_auroc"))}</td><td>{result["latency_ms"]["median"]:.2f}</td><td>{result["latency_ms"]["p95"]:.2f}</td><td>{_pct(loc.get("anomaly_non_empty_fraction"))}</td><td>{_pct(loc.get("normal_false_positive_fraction"))}</td><td><code>{result["threshold"]:.6f}</code></td></tr>"""
        )
    target_text = (
    ...
    env = html.escape(
        json.dumps(manifest["environment"], indent=2, default=_json_default)
    )
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if not candidates:
        raise ValueError(
            "Provide at least one model: --padim_model, --patchcore_model, or --efficientad_model."
        )
    ...
    if selected == "efficientad":
        for candidate in (source.with_suffix(".pth"), source.parent / "model.pth"):
            if candidate.exists():
                sidecar = output_dir / candidate.name
                shutil.copy2(candidate, sidecar)
                break
    if args.copy_config:
    ...
    manifest = {
        ...
        "calibration_artifact": sidecar.name if sidecar else None,
        "dataset": {
        ...
        json.dumps(
            {
                ...
                "report": str(
                    Path(args.output_dir).resolve() / "production_autopilot_report.html"
                ),
            },
            indent=2,
        )
