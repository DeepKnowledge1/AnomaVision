...
function LivePage({apiHealthy}:{apiHealthy:boolean}) {
  const [files,setFiles]=useState<File[]>([]); const [camera,setCamera]=useState(false); const [cameraBusy,setCameraBusy]=useState(false); const [cameraError,setCameraError]=useState(""); const videoRef=useRef<HTMLVideoElement>(null); const streamRef=useRef<MediaStream|null>(null); const [result,setResult]=useState<any>(null); const [busy,setBusy]=useState(false); const [error,setError]=useState(""); const [history,setHistory]=useState<any[]>([]); const [totalMs,setTotalMs]=useState(0);
  ...
  async function predictFrame(){if(!videoRef.current||cameraBusy)return;setCameraBusy(true);setCameraError("");try{const canvas=document.createElement("canvas");canvas.width=videoRef.current.videoWidth;canvas.height=videoRef.current.videoHeight;canvas.getContext("2d")?.drawImage(videoRef.current,0,0);const blob=await new Promise<Blob|null>(resolve=>canvas.toBlob(resolve,"image/jpeg",0.9));if(!blob)throw new Error("Could not capture camera frame");const body=new FormData();body.append("files",blob,"camera.jpg");const r=await fetch(`${inferenceUrl}/predict-batch`,{method:"POST",body});const d=await r.json();if(!r.ok)throw new Error(d.detail||"Camera inference failed");const rows=(d.batch_results||[]).map((x:any)=>({filename:"camera.jpg",...(x.result||{}),error:x.error}));setResult(d);setHistory(prev=>[...rows,...prev].slice(0,20));setTotalMs(rows.reduce((sum:number,x:any)=>sum+(Number(x.latency_ms)||0),0));}catch(e){setCameraError(e instanceof Error?e.message:"Camera inference failed")}finally{setCameraBusy(false)}}
  ...
  return <><div className="page-head"><div><div className="eyebrow">Inference</div><h1>Live</h1><p className="subtitle">Run images or a local folder through the existing AnomaVision inference runtime.</p></div></div>
    ...
    <div className="card camera-panel"><div className="section-head"><div><div className="section-title">Camera stream</div><div className="subtitle">Capture a frame from your browser camera and send it to the existing inference runtime.</div></div><div className="badge">{camera?"Live":"Off"}</div></div>{camera?<><video ref={videoRef} autoPlay playsInline muted className="camera-preview"/><div className="form-actions"><button className="primary" onClick={predictFrame} disabled={cameraBusy}><Play size={13}/>{cameraBusy?"Analyzing…":"Analyze frame"}</button><button className="secondary" onClick={stopCamera}>Stop camera</button></div></>:<button className="secondary" onClick={startCamera}><Wifi size={13}/> Start camera</button>}{cameraError&&<div className="form-error">{cameraError}</div>}</div>
