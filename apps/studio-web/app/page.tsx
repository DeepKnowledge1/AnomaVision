...
function LivePage({apiHealthy}:{apiHealthy:boolean}) {
  const [file,setFile]=useState<File|null>(null); const [result,setResult]=useState<any>(null); const [busy,setBusy]=useState(false); const [error,setError]=useState("");
  const inferenceUrl=process.env.NEXT_PUBLIC_ANOMAVISION_INFERENCE_URL ?? "http://localhost:8001";
  async function predict(){if(!file)return;setBusy(true);setError("");setResult(null);try{const body=new FormData();body.append("file",file);const r=await fetch(`${inferenceUrl}/predict?include_visualizations=true`,{method:"POST",body});const d=await r.json();if(!r.ok)throw new Error(d.detail||"Inference failed");setResult(d)}catch(e){setError(e instanceof Error?e.message:"Inference failed")}finally{setBusy(false)}}
  return <><div className="page-head"><div><div className="eyebrow">Inference</div><h1>Live</h1><p className="subtitle">Run a real image through the existing AnomaVision inference API.</p></div></div>
    <div className="grid-4"><Stat icon={<Wifi size={15}/>} label="Studio API" value={apiHealthy?"Online":"Offline"} meta="FastAPI connection"/><Stat icon={<CircleGauge size={15}/>} label="Inference API" value="External" meta={inferenceUrl}/><Stat icon={<Activity size={15}/>} label="Detector" value="Existing runtime" meta="no duplicate inference logic"/><Stat icon={<ShieldCheck size={15}/>} label="Drift" value="Observer" meta="does not change anomaly scores"/></div>
    <div className="card"><div className="section-head"><div><div className="section-title">Test inference</div><div className="subtitle">Upload one image and send it to the existing <code>/predict</code> endpoint.</div></div></div>
      <div className="form-grid"><label className="field"><span>Image</span><input type="file" accept="image/*" onChange={e=>setFile(e.target.files?.[0]||null)}/></label></div>
      <button className="primary" onClick={predict} disabled={!file||busy}><Play size={13}/>{busy?"Running…":"Run inference"}</button>
      {error&&<div className="form-error">{error}</div>}
      {result&&<div className="deployment-result"><div className="performance-grid"><div><span>Anomaly score</span><b>{Number(result.anomaly_score).toFixed(4)}</b></div><div><span>Classification</span><b>{result.is_anomaly?"Anomaly":"Normal"}</b></div></div>{result.heatmap_image_base64&&<img className="live-result-image" src={`data:image/png;base64,${result.heatmap_image_base64}`} alt="Anomaly heatmap"/>}</div>}
    </div>
  </>;
}
