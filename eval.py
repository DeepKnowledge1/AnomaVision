...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate anomaly detection model performance using trained models.")
    ...
    parser.add_argument('--viz_output_dir', type=str, default='./eval_visualizations/',
...
def evaluate_model_with_wrapper(model_wrapper, test_dataloader, logger, detailed_timing=False):
    """
    ...
    try:
        for batch_idx, (batch, images, image_targets, mask_targets) in enumerate(test_dataloader):
            ...
            all_image_classifications_target.extend(image_targets.numpy() if hasattr(image_targets, 'numpy') else image_targets)
            all_masks_target.extend(mask_targets.numpy() if hasattr(mask_targets, 'numpy') else mask_targets)
            ...
            if isinstance(image_scores, np.ndarray):
                ...
                all_image_scores.extend(image_scores.cpu().numpy().tolist() if hasattr(image_scores, 'cpu') else image_scores.tolist())
                all_score_maps.extend(score_maps.cpu().numpy() if hasattr(score_maps, 'cpu') else score_maps)
    ...
    return all_images, all_image_classifications_target, all_masks_target, all_image_scores, all_score_maps
...
def main(args):
    ...
    try:
        ...
        test_dataset = anodet.MVTecDataset(DATASET_PATH, args.class_name, is_train=False)
        ...
        logger.info(f"Test dataset created successfully. Total images: {len(test_dataset)}")
        logger.info(f"Batch size: {args.batch_size}, Number of batches: {len(test_dataloader)}")
    except Exception as e:
    ...
    logger.info(f"Starting evaluation of {len(test_dataset)} images using {model_type.value.upper()}")
    
    try:
        # Run evaluation using ModelWrapper
        images, image_classifications_target, masks_target, image_scores, score_maps = evaluate_model_with_wrapper(
    ...
    if args.enable_visualization:
        ...
        try:
            ...
            if args.save_visualizations:
                ...
                viz_filepath = os.path.join(args.viz_output_dir, f"evaluation_results_{args.class_name}_{timestamp}.png")
