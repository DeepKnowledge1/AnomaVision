...
class ModelType(Enum):
    ...
    def from_extension(cls, model_path):
        ...
        if os.path.isdir(model_path):
            ...
            for file in os.listdir(model_path):
                if file.endswith('.xml'):
                    return cls.OPENVINO
            raise ValueError(f"Directory {model_path} doesn't contain OpenVINO .xml files")
        ...
        if model_type is None:
            raise ValueError(f"Unsupported model format: {ext}. Supported: {list(extension_map.keys())} or OpenVINO directories")
