...
def _resize_maps_to_benchmark_size(maps: np.ndarray) -> np.ndarray:
    ...
    return F.interpolate(tensor, size=IMAGE_SIZE, mode="bilinear", align_corners=False)[:, 0].numpy()
...
def compute_auroc(image_labels, image_scores, masks, anomaly_maps) -> Tuple[float, float]:
...
def environment(device: torch.device, seed: int) -> Dict[str, str]:
    return {
        ...
        "seed": str(seed), "image_size": "224x224", "normalization": "ImageNet",
        "backbone": BACKBONE, "layers": ",".join(LAYERS), "n_features": str(N_FEATURES),
        "batch_size": str(BATCH_SIZE), "timing_batch_size": str(TIMING_BATCH_SIZE),
        "warmup_iters": str(WARMUP_ITERS), "timing_iters": str(TIMING_ITERS),
    }
...
class BenchmarkEngineMixin:
    def _setup_anomalib_callbacks(self) -> None:
        ...
        self._cache.args["callbacks"] = [
            callback for callback in self._cache.args["callbacks"]
...
class BenchmarkRunner:
    ...
    def _build_anomalib_datamodule(self, algorithm: str):
        ...
        kwargs = {
            "root": str(self.dataset_path), "category": self.class_name,
            "train_batch_size": 1 if algorithm == "efficientad" else BATCH_SIZE,
            "eval_batch_size": BATCH_SIZE, "num_workers": 0, "seed": self.seed,
        }
    ...
    def _benchmark_latency(self, model, batch: torch.Tensor, call_model) -> Tuple[float, float, float, float]:
        model.eval()
    ...
    def _build_anomavision_model(self, algorithm: str):
        if algorithm == "padim":
            ...
            return Padim(backbone=BACKBONE, device=self.device, feat_dim=N_FEATURES, layer_indices=LAYER_INDICES)
        if algorithm == "patchcore":
            from anomavision import PatchCore
            return PatchCore(
                backbone=BACKBONE, device=self.device, layer_indices=LAYER_INDICES,
                coreset_ratio=0.02, max_memory_patches=2048, patch_grid=14,
                n_neighbors=1, coreset_seed=self.seed,
            )
        if algorithm == "efficientad":
            from anomavision import EfficientAD
            return EfficientAD(backbone=BACKBONE, device=self.device, epochs=5, learning_rate=1e-3)
    ...
    def _build_anomalib_model(self, algorithm: str):
        ...
        if algorithm == "padim":
            return Padim(backbone=BACKBONE, layers=LAYERS, pre_trained=True, n_features=N_FEATURES)
        if algorithm == "patchcore":
            return Patchcore(
                backbone=BACKBONE, layers=LAYERS, pre_trained=True,
    ...
    def _evaluate_anomavision(self, model, test_loader) -> Tuple[float, float]:
        ...
        with torch.inference_mode():
            for batch in test_loader:
                ...
                image_labels.append(tensor_to_numpy(labels)); image_scores.append(tensor_to_numpy(scores))
                masks.append(tensor_to_numpy(gt_masks)); maps.append(tensor_to_numpy(anomaly_maps))
        return compute_auroc(np.concatenate(image_labels), np.concatenate(image_scores), np.concatenate(masks), np.concatenate(maps))
    ...
    def benchmark_anomavision(self, algorithm: str) -> ModelMetrics:
        ...
        metrics = ModelMetrics(f"AnomaVision {algorithm.title()}", algorithm, device=str(self.device), environment=environment(self.device, self.seed))
        print(f"\nANOMAVISION {algorithm.upper()} | Train: {len(train_dataset)} | Test: {len(test_dataset)}")
        self._reset_memory()
        start = time.perf_counter(); model.fit(train_loader); sync(self.device)
        metrics.training_time_s = time.perf_counter() - start
        metrics.training_memory_mb = self._memory_now_mb()
        metrics.state_dict_size_mb = self._state_dict_size_mb(model, self.output_dir / f"anomavision_{algorithm}_{self.class_name}_state_dict.pt")
        ...
        metrics.image_auroc, metrics.pixel_auroc = self._evaluate_anomavision(model, test_loader)
        print(f"  image={metrics.image_auroc:.4f} pixel={metrics.pixel_auroc:.4f} latency={metrics.latency_ms:.2f}ms")
        return metrics
    ...
    def benchmark_anomalib(self, algorithm: str) -> ModelMetrics:
        ...
        train_loader = datamodule.train_dataloader(); test_loader = datamodule.test_dataloader()
        model = self._build_anomalib_model(algorithm).to(self.device)
        metrics = ModelMetrics(f"Anomalib {algorithm.title()}", algorithm, device=str(self.device), environment=environment(self.device, self.seed))
        print(f"ANOMALIB {algorithm.upper()} | Train: {len(train_loader.dataset)} | Test: {len(test_loader.dataset)}")
        engine_cls = type("BenchmarkEngine", (BenchmarkEngineMixin, Engine), {})
        engine = engine_cls(
            max_epochs=1, accelerator="gpu" if self.device.type == "cuda" else "cpu", devices=1,
            logger=False, enable_progress_bar=False, enable_checkpointing=False,
        )
        self._reset_memory()
        start = time.perf_counter(); engine.fit(model=model, datamodule=datamodule); sync(self.device)
        metrics.training_time_s = time.perf_counter() - start
        metrics.training_memory_mb = self._memory_now_mb()
        metrics.state_dict_size_mb = self._state_dict_size_mb(model, self.output_dir / f"anomalib_{algorithm}_{self.class_name}_state_dict.pt")
        timing_batch = _get_anomalib_batch_value(next(iter(test_loader)), "image")[:TIMING_BATCH_SIZE]
        ...
        metrics.image_auroc, metrics.pixel_auroc = self._evaluate_anomalib(model, test_loader)
        print(f"  image={metrics.image_auroc:.4f} pixel={metrics.pixel_auroc:.4f} latency={metrics.latency_ms:.2f}ms")
        return metrics
    ...
    def report(self, results: List[ModelMetrics]) -> None:
        ...
        summary = frame[["algorithm", "name", "image_auroc", "pixel_auroc", "training_time_s", "latency_ms", "p95_latency_ms", "throughput_fps", "state_dict_size_mb", "training_memory_mb", "inference_memory_mb"]].copy()
        with open(f"{stem}.txt", "w", encoding="utf-8") as handle:
            handle.write("AnomaVision vs Anomalib Benchmark\n")
            handle.write("=" * 80 + "\n\n")
            handle.write(tabulate(summary, headers="keys", tablefmt="github", showindex=False, floatfmt=".4f"))
            handle.write("\n")
        html_rows = []
        for row in rows:
            html_rows.append(
                "<tr>" + "".join(f"<td>{row[k]}</td>" for k in ["algorithm", "name", "image_auroc", "pixel_auroc", "training_time_s", "latency_ms", "p95_latency_ms", "throughput_fps", "state_dict_size_mb"]) + "</tr>"
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark AnomaVision against a local Anomalib checkout.")
    ...
    BenchmarkRunner(args.dataset_path, args.class_name, args.device, args.seed).run(args.algorithms)
