...
$inferenceApiCommand = "Set-Location '$Root'; `$env:PORT='8001'; uv run python apps\api.py"
...
if ($inferenceReady) {
    ...
    Write-Host "The inference service failed to start. Check its PowerShell window for the startup error." -ForegroundColor Red
}
